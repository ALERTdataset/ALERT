    ...
    def __init__(self, time_file, freq_file, sample_size): # sample_size is observation length (4 s)
        ...
        for time_path, freq_path in zip(time_file, freq_file):
            ...
            for i in range(n_samples):  
                if i <= n_samples-2:
                    self.time.append(time_mat[rangebin_start:rangebin_end, int(i*sample_size/2):int(((i+1)*sample_size/2)+(sample_size/2))])
                    self.freq.append(freq_mat[rangebin_start:rangebin_end, int(i*sample_size/2):int(((i+1)*sample_size/2)+(sample_size/2))])
                    tmp = []
                    ...
                    for i in range(n_samples):
                        ...
                        #self.time.append(time_mat[rangebin_start:rangebin_end, i*sample_size:(i+1)*sample_size])
                                ...
                                if sys.argv[2] == 'CA':
                                    ...
                                    subject_freq[task][self.label_idx].append(freq_mat[45:89, i*sample_size:(i+1)*sample_size])
                                elif sys.argv[2] == 'cropO':
                                    subject_time[task][self.label_idx].append(time_mat[rangebin_start:rangebin_end, i*sample_size:(i+1)*sample_size])
                                    subject_freq[task][self.label_idx].append(freq_mat[0:89, i*sample_size:(i+1)*sample_size])
                                elif sys.argv[2] == 'RD':
                                    fft_result = np.fft.fft(time_mat[rangebin_start:rangebin_end, i*sample_size:(i+1)*sample_size], axis=1)
                                    subject_time[task][self.label_idx].append(np.abs(fft_result))
                                    subject_freq[task][self.label_idx].append(freq_mat[0:89, i*sample_size:(i+1)*sample_size])
...
if __name__ == "__main__":
    ...
    try:
        if sys.argv[2] == 'cropO' or sys.argv[2] == 'RD':
            # Only apply to temporal data
            rangebin_start = 120 # y-axis of input 0, 100 
            rangebin_end = 171 # y-axis of input 172, 177
        elif sys.argv[2] == 'cropX':
            rangebin_start = 0 # y-axis of input 0, 100 
            rangebin_end = 177 # y-axis of input 172, 177
        elif sys.argv[2] == 'CA':
            rangebin_start = 140
            rangebin_end = 171
        elif sys.argv[2] == 'CA_fft':
